  ...
  async restore() {
    ...
    try {
      ...
      if (response.length === 0) {
        Alert.alert(
          I18n.t('restorePurchaseNotFoundTitle'),
          I18n.t('restorePurchaseNotFoundMessage')
        );
        return;
      }
      await fetchMyCourse(true);
      Alert.alert(
        I18n.t('restorePurchaseSuccessTitle'),
        I18n.t('restorePurchaseSuccessMessage')
      );
    } catch (error) {
      console.error(error);
      Alert.alert(
        I18n.t('restorePurchaseErrorTitle'),
        I18n.t('restorePurchaseErrorMessage')
      );
    } finally {
      ...
      <View style={[styles.rowWrapper, rowStyle]}>
        ...
        <View style={styles.fieldValueWrapper}>
          <Text
            style={[
              styles.fieldText,
              _.isEmpty(fieldValue) ? { color: '#999' } : {},
              valueStyle
            ]}
          >
        ...
        {user.isTemporary === false && ( // 購入済みゲストユーザーにはログインさせない
          <View style={styles.forGuestFieldWrapper}>
            <View style={styles.forGuestFieldLableTextWrapper}>
              <Text style={styles.forGuestFieldLableText}>
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.profileContainer, isGuest && { flex: 0.5 }]}>
          <View style={styles.labelWrapper}>
            <Text style={styles.label}>{I18n.t('profile')}</Text>
          </View>
          <View style={styles.profileContentWrapper}>
            ...
            {!isGuest &&
              this.renderProfileField(I18n.t('nickName'), user.nickName)}
            {!isGuest &&
              this.renderProfileField(I18n.t('email'), user.email, {
                borderBottomWidth: 0
              })}
          </View>
        </View>
          ...
          {Platform.OS === 'ios' && (
            <Button
              containerStyle={[
                ...
                styles.restoreButtonWrapper
              ]}
              style={[styles.buttonText, styles.restoreButtonText]}
              onPress={() => (isOnline ? this.restore() : alertOfflineError())}
