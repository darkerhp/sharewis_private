...
import {alert as alertOfflineError} from '@utils';
import {baseStyles} from '@lib';
import {TextField} from '@components';
import { validate as validateEmailLogin} from '@utils';
      ...
      if (/email/.test(error.message)) {
        Alert.alert(
          ...
          [
            {
              text: I18n.t('login'),
              onPress: () => RouterActions.login({ initialEmailValue: email })
            },
            { text: I18n.t('cancel') }
          ]
        );
    ...
    return (
      <View style={styles.container}>
        <View style={styles.labelWrapper}>
          <Text style={styles.label}>{I18n.t('signupWithEmail')}</Text>
        </View>
